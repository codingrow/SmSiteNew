 ...
 */
class File {
    static $file_mime_arr;
    public static function guess_extension($string) {
        $string = strrev($string);
        return '.' . strrev(stristr($string, '.', true));
    }

    public static function verify_extension($extension, $mime) {
        $extension = ltrim($extension, '.');
        if (array_key_exists($extension, static::$file_mime_arr)) {
            if (is_array(static::$file_mime_arr[$extension]) && in_array($mime, static::$file_mime_arr[$extension])) {
                return true;
            } else {
                if (static::$file_mime_arr[$extension] == $mime) {
                    return true;
                }
            }
        }
...
#@todo(sam_washington) implement this externally, cache if possible
File::$file_mime_arr = [
    '3g2' =>	'video/3gpp2',
    '3gp' =>	'video/3gp',
